...
const ContactPage = () => {
  ...
  const sendData = () => {
    axios.post('http://188.225.83.42:8001/api/v1/site/feedback/', { name, email, subject, text })
      ...
      .catch(e => setError({
        ...
        subject: e.response.data.errors.subject ? e.response.data.errors.subject[0] : '',
  ...
  return (
    <Wrapper>
      <WrapperInner>
        ...
        <InputField style={error?.name ? { border: '1px solid red' } : {}} value={name}
                    onChange={(e) => setName(e.target.value)}
                    placeholder='Ваше имя' />
        {error?.email && <ErrorText>{error.email}</ErrorText>}
        <InputField style={error?.email ? { border: '1px solid red' } : {}} value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder='Ваш email' />
        {error?.subject && <ErrorText>{error.subject}</ErrorText>}
        <Select style={error?.subject ? { border: '1px solid red' } : {}}
                onChange={(e) => setSubject(e.target.value)}>
          <option value='' hidden>Тема сообщения</option>
          <option value='жалоба'>Жалоба</option>
          <option value='предложение'>Предложение</option>
        </Select>
        {error?.text && <ErrorText>{error.text}</ErrorText>}
        <StyledTextarea style={error?.text ? { border: '1px solid red' } : {}} value={text}
                        onChange={(e) => setText(e.target.value)}
                        placeholder='Вашe сообщение' />
        <Button onClick={sendData} type='button'>отправить сообщение</Button>
      </WrapperInner>
    </Wrapper>
  );
};
