...
const AddListComponent: React.FC<IProps> = ({ result, page, limit }) => {
  ...
  const deleteAdvert = (id: number) => {
    ...
    axios
      .delete(`http://188.225.83.42:8001/api/v1/advertisement/${id}/`, {
        headers: {
        ...
        {data?.goods?.map((item: IResult) => {
          return (
            <AddListUlInfo key={item.id}>
              ...
              <AddListItem className="button">
                <svg onClick={() => deleteAdvert(item.id)} className="icon">
                  {' '}
                  {Delete}
                </svg>
              </AddListItem>
