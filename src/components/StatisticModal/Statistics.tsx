...
const Statistics = ({ result, statistics, setStatictics }: ResultType) => {
  ...
  const fetchData = () => {
    ...
    for (let i = 0; i < Object.keys(result).length; i++) {
      if (
        Object.keys(result)[i] !== 'phone_views_count' &&
        Object.keys(result)[i] !== 'views_count'
      ) {
        res.push({
  ...
  const renderCustomizedLabel = (props: any) => {
    ...
    return (
      <g>
        <text fill="#828282" x={x + 1} y={y - 6}>
          {data[index]?.uv + data[index]?.pv}
        </text>
      </g>
    );
  };
  ...
  return (
    <StatisticsSection>
      ...
      <StatisticsInner>
        <StatisticsHead>
          <StatisticsTitle>
            Статистика просмотров объявления за месяц
          </StatisticsTitle>
          <svg
            onClick={onCloseStatic}
            width={13}
            height={13}
            viewBox="0 0 13 13"
            fill="none"
          ...
          <StatisticsRight>
            ...
            <StatisticsView>
              <p>
                ...
                <span>
                  {' '}
                  {typeof result !== 'number' && result.phone_views_count}
                </span>
              </p>
              <p>
                Просмотров всего:
                <span> {typeof result !== 'number' && result.views_count}</span>
              </p>
            </StatisticsView>
            <StatisticsBurn>
              <svg
              ...
              <StatisticsBurnSubtitle>
                Купите услуги продвижения магазина
              </StatisticsBurnSubtitle>
